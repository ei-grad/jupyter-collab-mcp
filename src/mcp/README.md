...
- `notebook_execute.wait_ms` waits for terminal completion or its deadline;
  intermediate kernel updates do not end the wait. After a timeout, follow
  the returned cursor with `execution_get`, which still waits for the next
  update. Waiting never holds the mutation lock or resends code.
